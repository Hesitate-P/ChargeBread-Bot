...
# 昵称净化：这些字符能组成 markdown 结构。出现在昵称里就会破坏我们的排版，
# 或者注入内容（`![](url)` 能往消息里塞图片，`[文字](url)` 能塞链接）。
# 中文、英文、emoji、下划线一律保留。
#
# 特意**不删下划线**：下划线是用户名里的常见字符（`Hesitate_P`），删了会把
# 真实昵称改坏；而保留它的最坏后果只是名字呈现为斜体 —— 纯外观，注入不了
# 任何东西。外观风险远小于改坏名字。
...
def _badge(rank: int) -> str:
    ...
    实测建议序号与奖牌并存：原来前三名只给奖牌、第 4 名突然变成 "4."，
    视觉上断裂，也不好数。
    """
